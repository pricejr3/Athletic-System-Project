...
sub view_form
{
	...
	if (defined ($cgi_data->{'BULK_VIEW'})) {   # need to find the latest instance of the form (ORIG_VERSION_ID) for the current athlete
	    ...
	    $cgi_data->{'ORIG_VERSION_ID'} = get_latest_athlete_orig_version_id ($table_name, $orig_athlete_id,
										 $cgi_data->{'FORM_SIGN_DATE1'},
										 $cgi_data->{'FORM_SIGN_DATE2'});
...
{
    my ($table_name, $orig_athlete_id, $form_sign_date1, $form_sign_date2) = @_;
...
WHERE
  ATHLETE_ID = '$orig_athlete_id'";
    ...
    if ($form_sign_date1) {
	$sql .= " and FORM_SIGN_DATE >= TO_DATE('$form_sign_date1', 'MM-DD-YYYY')";
    }

    if ($form_sign_date2) {
	$sql .= " and FORM_SIGN_DATE <= TO_DATE('$form_sign_date2', 'MM-DD-YYYY')";
    }
