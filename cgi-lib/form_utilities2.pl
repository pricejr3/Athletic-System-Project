...
sub view_form
{
	...
	foreach my $key (keys (%$cgi_data)) {
	    # Looking for format: ORIG_ATHLETE_ID_{athlete ID}
	    if ($key =~ /^ORIG_ATHLETE_ID_(\d+)$/) { $orig_athlete_ids{$1} = 1; }
	}

    } else {  # viewing a form for a single athlete
	$orig_athlete_ids{$cgi_data->{'ORIG_ATHLETE_ID'}} = 1;
    }
    ...
    foreach my $orig_athlete_id (keys (%orig_athlete_ids)) {
	$n++;
	...
	if (defined ($cgi_data->{'BULK_VIEW'})) {   # need to find the latest instance of the form (ORIG_VERSION_ID) for the current athlete
	    ...
	    $cgi_data->{'ORIG_VERSION_ID'} = get_latest_orig_version_id ($table_name, $orig_athlete_id);
...
sub get_latest_orig_version_id
{
    my ($table_name, $orig_athlete_id) = @_;
